...
var DemandlettersComponent = /** @class */ (function () {
  function DemandlettersComponent(
    route,
    ecolService,
    spinner,
    toasterService,
    ...
    this.uploader.onSuccessItem = function (item, response, status, headers) {
      ...
      for (var i = 0; i < obj.files.length; i++) {
        ...
        _this.ecolService.demandshistory(bulk).subscribe(
          function (resp) {
    ...
    this.ecolService.getAccount(this.accnumber).subscribe(
      function (data) {
        ...
        if (data && data.length > 0) {
          ...
          _this.ecolService.getcustwithAccount(data[0].custnumber).subscribe(
            function (accounts) {
              // add accounts to the array
              // console.log('accounts=>', accounts);
              _this.bodyletter.accounts = accounts;
              // get demand1 date
              _this.ecolService.demand1history(_this.accnumber).subscribe(
                function (dd1date) {
                  if (dd1date && dd1date.length > 0) {
                    _this.bodyletter.demand1date = dd1date[0].datesent;
                  }
                  // call generate letter api
                  _this.ecolService.generateLetter(_this.bodyletter).subscribe(
                    function (generateletterdata) {
                      ...
                      if (generateletterdata.result === 'success') {
                        ...
                        if (_this.model.previewtosend) {
                          ...
                          _this.ecolService
                            ...
                            .subscribe(
                              function (respo) {
                                ...
                                _this.smsMessage = sms.replace(
                                  '[emailaddressxxx]',
                                  'email address ' + _this.model.emailaddress
                                );
                                var smsdata = {
                                  demand: letter.demand,
                                  custnumber: _this.model.custnumber,
                          ...
                          _this.ecolService.demandstatus(status_1).subscribe(
                            function (ddstatusdata) {
    ...
    this.ecolService.getcardAccount(this.accnumber).subscribe(
      function (carddata) {
        ...
        if (carddata && carddata.length > 0) {
          ...
          // call generate letter api
          _this.ecolService.generateLetter(_this.letterbody).subscribe(
            function (data) {
              // sucess
  ...
  DemandlettersComponent.prototype.processletter = function (
    letter,
    accnumber,
    emailaddress
  ) {
    ...
    this.ecolService.getAccount(accnumber).subscribe(
      function (data) {
        if (data && data.length > 0) {
          ...
          _this.bodyletter.guarantors = data[0].guarantors;
          _this.bodyletter.settleaccno =
            data[0].settleaccno || '00000000000000';
                ...
                this.ecolService
                  ...
                  .subscribe(function (response) {
                    if (response.result === 'fail') {
                      ...
                      _this.getdemandshistory(_this.accnumber);
                      // send sms
                      _this.ecolService.getsmsmessage(letter.demand).subscribe(
                        function (respo) {
                          var sms = respo.smstemplate;
                          _this.smsMessage = sms.replace(
                            '[emailaddressxxx]',
  ...
  DemandlettersComponent.prototype.processlettercc = function (
    demand,
    cardacct,
    emailaddress
  ) {
  ...
  DemandlettersComponent.prototype.generatelettercc = function (
    letter,
    emaildata
  ) {
    var _this = this;
  ...
  DemandlettersComponent.prototype.downloadDemand = function (
    filepath,
    filename
  ) {
