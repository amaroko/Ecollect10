...
var BulknotesComponent = /** @class */ (function () {
  ...
  BulknotesComponent.prototype.onSuccessItem = function (
    item,
    response,
    status,
    headers
  ...
  BulknotesComponent.prototype.onErrorItem = function (
    item,
    response,
    status,
    headers
  ...
  BulknotesComponent.prototype.onFileChange = function (ev) {
    ...
    reader.onload = function (event) {
      ...
      for (var i = 0; i < jsonData.Sheet1.length; i++) {
        // check for null
        if (
          _this.outdata[i].accnumber == null ||
          ...
          _this.outdata[i].owner = _this.username;
          _this.outdata[i].custnumber = _this.outdata[i].accnumber.substring(
            5,
            12
          );
          _this.outdata[i].notesrc = 'uploaded a note';
      ...
      swal
        ...
        .then(function (result) {
          if (result.value) {
            ...
            _this.ecolService.bulknotes(_this.outdata).subscribe(
              function (events) {
                if (events.type === HttpEventType.UploadProgress) {
                  _this.fileUploadProgress = Math.round(
                  ...
                  if (_this.sys === 'cc' || _this.sys === 'watchcc') {
                    ...
                    //
                    _this.ecolService
                      .bulktotblportfolio(_this.outdata)
                      .subscribe(
                        function (results) {
                          console.log(results);
                          _this.sendNotesData(_this.custnumber); // updates the notes counter on view
  ...
  __decorate(
    [ViewChild('myInput'), __metadata('design:type', ElementRef)],
    BulknotesComponent.prototype,
    'myInputVariable',
    void 0
  );
