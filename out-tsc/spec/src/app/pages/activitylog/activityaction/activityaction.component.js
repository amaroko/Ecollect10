...
var ActivityactionComponent = /** @class */ (function () {
  function ActivityactionComponent(
    el,
    route,
    formBuilder,
    ecolService,
  ...
  ActivityactionComponent.prototype.getmcoop = function (loanaccnumber) {
    var _this = this;
    this.ecolService
      .getmcoopcashAccount(loanaccnumber)
      .subscribe(function (data) {
        _this.account = data[0];
        _this.loader = false;
      });
  ...
  ActivityactionComponent.prototype.getexcuse = function () {
    ...
    this.ecolService.getexcuse().subscribe(function (data) {
      _this.excuse = data;
      _this.mappedexcuse = Array.from(
        new Set(
          _this.excuse.map(function (_a) {
            var EXCUSE = _a.EXCUSE;
            return EXCUSE;
          })
        )
      );
      _this.mappedid = Array.from(
        new Set(
          _this.excuse.map(function (_a) {
            var ID = _a.ID;
            return ID;
      ...
      _this.mappedexcusedetails = Array.from(
        new Set(
          _this.excuse.map(function (_a) {
    ...
    this.ecolService.postactivitylogs(this.savebody).subscribe(
      function (data) {
        ...
        if (_this.sys === 'watchcc') {
          ...
          _this.ecolService.putcardwatch(watchccbody).subscribe(
            function (resp) {
              //
            },
            function (error) {
        ...
        if (_this.sys === 'watch') {
          ...
          _this.ecolService.putwatch(watchbody).subscribe(
            function (resp) {},
            function (error) {
              console.log(error);
            }
        ...
        if (_this.sys === 'ptp') {
          ...
          _this.ecolService.reviewptp(ptpbody).subscribe(
            function (resp) {},
            function (error) {
              console.log(error);
            }
  ...
  ActivityactionComponent.prototype.changePtp = function (value) {
    ...
    if (value === 'Yes') {
      // check if ptp exists
      this.ecolService
        .activeptps(this.accnumber)
        .subscribe(function (activedata) {
  ...
  __decorate(
    [ViewChild('reason'), __metadata('design:type', ElementRef)],
    ActivityactionComponent.prototype,
    'inputOne',
    void 0
  );
