  ...
  PtpsComponent.prototype.openModal = function (id) {
    ...
    this.ecolService.getthisptp(id).subscribe(
      function (data) {
        ...
        _this.maxyear = parseInt(
          moment(_this.ammendptp.ptpdate).add(5, 'days').format('YYYY')
        );
        _this.minyear = parseInt(
          moment(_this.ammendptp.ptpdate).subtract(5, 'days').format('YYYY')
        );
        _this.month = parseInt(moment(_this.ammendptp.ptpdate).format('MM'));
        _this.maxmonth = parseInt(
          moment(_this.ammendptp.ptpdate).add(5, 'days').format('MM')
        );
        _this.minmonth = parseInt(
          moment(_this.ammendptp.ptpdate).subtract(5, 'days').format('MM')
        );
        _this.day = parseInt(moment(_this.ammendptp.ptpdate).format('DD'));
        _this.maxday = parseInt(
          moment(_this.ammendptp.ptpdate).add(5, 'days').format('DD')
        );
        _this.minday = parseInt(
          moment(_this.ammendptp.ptpdate).subtract(5, 'days').format('DD')
        );
        _this.maxDate = {
          year: _this.maxyear,
    ...
    swal
      ...
      .then(function (result) {
        if (result.value) {
          ...
          _this.ecolService.ammendptp(body).subscribe(
            function (resp) {
              //
