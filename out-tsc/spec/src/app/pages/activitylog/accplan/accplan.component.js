    ...
    if (planid) {
      ...
      this.ecolService.s_plan_actions(planid).subscribe(
        function (resp) {
          // console.log(resp);
          // tbl_s_account_plans
          _this.ecolService.s_account_plans(_this.accnumber, planid).subscribe(
            function (data) {
      ...
      .then(function (result) {
        if (result.value) {
          for (var i = 0; i < _this.currentplan.length; i++) {
            _this.currentplan[i].datecompleted = moment(
              _this.currentplan[i].datecompleted
            ).format('YYYY-MM-DD');
          ...
          if (_this.update) {
            ...
            for (var i = 0; i < _this.currentplan.length; i++) {
              _this.ecolService.putaccountplan(_this.currentplan[i]).subscribe(
                function (data) {},
                function (error) {
                  console.log(error);
                }
            ...
            // console.log('this.currentplan', this.currentplan);
            // console.log(acc);
            _this.ecolService.saveaccountplan(_this.currentplan).subscribe(
              function (data) {
                alert('plan saved');
                console.log(data);
                _this.changeAction(_this.currentplan[0].planid);
